...
- **The text wrapper must hug the 6u line-box, not add a strut.** Set the wrapper to flex (`display:flex; align-items:center`) or give the wrapper the child's own `line-height: 6u`. A bare block/`inline-block` wrapper emits a font *strut* from inherited metrics — its line-box grows past 6u, the band inflates, and emergent height overshoots H (a 6u band becomes 8u, H=24 renders at 28). The child text keeps line-height 6u; the wrapper flexes to that exact box. This is the single most common break of emergent height with a text label — verify by measuring the built container's height equals H.
